        ...
        <div className="mb-6">
          {documentQuery.isLoading ? (
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {doc?.aiModel && (
                <span
                  className="inline-flex items-center gap-1 rounded-full bg-violet-100 dark:bg-violet-950 px-2.5 py-0.5 text-xs font-medium text-violet-700 dark:text-violet-300"
                  title={`Generated by ${doc.aiModel}`}
                >
                  {/* Strip the ":free" suffix — it's an OpenRouter routing tag, not meaningful to users */}
                  ✦ {doc.aiModel.replace(/:free$/, '')}
                </span>
              )}
            </div>
          )}
