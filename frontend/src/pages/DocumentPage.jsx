...
function ModelBadge({ model }) {
  ...
  // Strip the ":free" OpenRouter routing suffix — not meaningful to users
  const label = model.replace(/:free$/, '')
  return (
    <p className="mt-4 text-xs text-gray-400 dark:text-gray-600 flex items-center gap-1">
      <span>✦</span>
      <span>Generated by <span className="font-medium">{label}</span></span>
    </p>
  )
}
...
function SummaryTab({ data, isLoading, isError, model }) {
  if (isLoading) {
    return (
...
function FlashcardsTab({ data, isLoading, isError, model }) {
  if (isLoading) {
    return (
...
function QuizTab({ data, isLoading, isError, model }) {
  if (isLoading) {
    return (
