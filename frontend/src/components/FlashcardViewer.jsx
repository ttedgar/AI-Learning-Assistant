...
export default function FlashcardViewer({ flashcards }) {
  ...
  return (
    <div className="flex flex-col items-center gap-6">
      {/* Progress counter */}
      <p className="text-sm text-gray-500 font-medium" data-testid="flashcard-progress">
        {index + 1} / {total}
      </p>
      ...
      >
        ...
        >
          {/* Front face — question */}
          <div
            className="absolute inset-0 flex flex-col items-center justify-center p-8 bg-white border border-gray-200 rounded-2xl shadow-sm"
            style={{ backfaceVisibility: 'hidden' }}
          >
            <p className="text-xs text-gray-400 uppercase tracking-wide mb-3">Question</p>
            <p className="text-gray-900 text-center font-medium leading-relaxed">{card.question}</p>
            <p className="text-xs text-gray-400 mt-6">Click to reveal answer</p>
          </div>

          {/* Back face — answer */}
          <div
            className="absolute inset-0 flex flex-col items-center justify-center p-8 bg-indigo-50 border border-indigo-100 rounded-2xl shadow-sm"
            style={{ backfaceVisibility: 'hidden', transform: 'rotateY(180deg)' }}
          >
            <p className="text-xs text-indigo-400 uppercase tracking-wide mb-3">Answer</p>
            <p className="text-gray-900 text-center font-medium leading-relaxed">{card.answer}</p>
          </div>
        </div>
      </div>
